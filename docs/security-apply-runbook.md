...
### 2026-09-20, inferred search tags applied

- `migrations/product-search-tags-2026-09-20.sql`: **applied**, creating
  `public.product_search_tag`. It had first passed 24 checks on a local
  PostgreSQL replica (`scripts/replica_search_tags_test.py`, now a CI step),
  including the undo and a re-apply after the undo.
- A second, accidental run of the migration was refused by its own preflight
  with `Search tags are already applied` and changed nothing, which is the
  guard working as intended.
- **Verification 9/9**, including that no client role — anon, authenticated or
  service_role — holds INSERT, UPDATE or DELETE on the table, and that every
  policy is limited to published products.
- `seed/search-tags-2026-09-20.sql`: **applied**. 277 inferred tags across all
  45 eligible products, generated by `scripts/derive_search_tags.py` from each
  product's own catalogue entry and primary photograph, two runs per product
  with only what both agreed on kept. The file counts its own rows and rolls
  back if the number is wrong.
- Read back through PostgREST as an anonymous visitor immediately afterwards:
  277 rows over 45 products, every one `source = 'ai_inferred'` and
  `model_name = 'gemini-3.6-flash'`. That is the same path the API reads them
  by, so fuzzy ranking is now live rather than inert.
- These are the first rows this project has written that are not marketplace
  facts. They rank and never filter, cap below a seller-confirmed attribute,
  and every reason built from one is labelled a guess.
- Undo: `migrations/product-search-tags-2026-09-20-undo.sql` drops the table
  and every tag with it. Nothing depends on it; the API treats a missing tag
  table exactly as an empty one.
