...
- `seed/phase-5-fake-catalogue.sql` and `seed/phase-5-fake-catalogue-remove.sql`
  are rendered from the same data for the fake-data testing branch. A test
  asserts the files on disk are byte-identical to the renderer; never hand-edit
  them.
...
are reproducible. Images point at `https://placehold.co` placeholders.
