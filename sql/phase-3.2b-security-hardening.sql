...
USING (
    EXISTS (
        ...
        WHERE p.id = product_color.product_id
          AND (
              p.lifecycle_state =
                  'published'::public.product_state
              OR p.marketplace_party_id =
...
USING (
    EXISTS (
        SELECT 1
        FROM public.product AS p
        WHERE p.id = product_3d_model.product_id
          AND (
              p.lifecycle_state =
                  'published'::public.product_state
              OR p.marketplace_party_id =
...
USING (
    EXISTS (
        SELECT 1
        FROM public.product AS p
        WHERE p.id = product_enrichment_assignment.product_id
          AND (
              p.lifecycle_state =
                  'published'::public.product_state
              OR p.marketplace_party_id =
...
-- Child reads follow the parent product's public or owner visibility. The
...
DECLARE
    ...
    maintain_privilege_remains boolean;
    expected_tables constant text[] := ARRAY[
        'address',
        'admin_user',
        'cart',
...
BEGIN
    ...
    IF pg_catalog.pg_has_role(
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: client role inherits or can assume service_role';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: dangerous PUBLIC, anon, or authenticated privilege remains';
    END IF;

    -- MAINTAIN is unknown to PostgreSQL 15/16. A nested IF prevents the
    -- version-specific statement from even being parsed there; do not rely on
    -- Boolean evaluation order to protect the privilege parser.
    IF current_setting('server_version_num')::integer >= 170000 THEN
        EXECUTE $maintain_check$
            SELECT EXISTS (
                SELECT 1
                FROM pg_catalog.pg_class AS relation
                WHERE relation.relnamespace = 'public'::regnamespace
                  AND relation.relkind IN ('r', 'p')
                  AND (
                      pg_catalog.has_table_privilege($1, relation.oid, 'MAINTAIN')
                      OR pg_catalog.has_table_privilege($2, relation.oid, 'MAINTAIN')
                      OR EXISTS (
                          SELECT 1
                          FROM pg_catalog.aclexplode(
                              COALESCE(
        ...
        $maintain_check$
        INTO maintain_privilege_remains
        USING anon_role_oid, authenticated_role_oid;

        IF maintain_privilege_remains THEN
            RAISE EXCEPTION USING
    ...
    -- Recheck the six retained mixed policies by catalog metadata,
    -- dependencies, and critical ingredients after the explicit ALTERs.
    FOR policy_expectation IN
        SELECT *
        FROM (VALUES
            (
    ...
    LOOP
        ...
        IF policy_row.polcmd <> 'r'::pg_catalog."char"
           ...
           OR policy_row.using_expr IS NULL
           OR canonical_policy_expression IS DISTINCT FROM
               policy_expectation.expected_canonical_expression
           OR or_operator_position = 0
           OR (
           ...
           OR position('is_admin' IN lower(policy_row.using_expr)) > 0
           OR NOT EXISTS (
               SELECT 1
               FROM pg_catalog.pg_depend AS dependency
               WHERE dependency.classid = 'pg_catalog.pg_policy'::regclass
                 AND dependency.objid = policy_row.policy_oid
                 AND dependency.refclassid = 'pg_catalog.pg_proc'::regclass
                 AND dependency.refobjid =
                     'public.current_marketplace_party_id()'::regprocedure
           )
        ...
        THEN
            RAISE EXCEPTION USING
                MESSAGE = format(
                    'Phase 3.2B postflight: mixed-policy mismatch public.%s.%s',
                    policy_expectation.table_name,
                    policy_expectation.policy_name
                );
        END IF;
    ...
    -- The core migration never targets storage. Recheck its complete reviewed
    -- non-owner default-ACL signature before COMMIT so global function-default
    -- hardening cannot silently remove or broaden managed access.
    IF EXISTS (
        WITH expected_storage_signature(
            owner_name,
            schema_name,
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: managed storage default-ACL signature drift';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: unreviewed postgres role default-ACL scope';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: managed storage function default EXECUTE is not effective';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            MESSAGE = 'Phase 3.2B postflight: explicit service_role future-function EXECUTE missing';
    END IF;
END
$phase32b_postflight$;
